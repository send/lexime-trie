...
mod tests {
    ...
    fn predictive_search_pop_cap_bounds_cyclic_corruption() {
        // Hand-built corrupt trie whose child windows point into a cycle.
        // Without the pop cap on `PredictiveIter` this would not
        // terminate — this test doubles as a hang detector.
        use crate::code_map::CodeMapper;
        use crate::Node;

        // Alphabet size 3: `base ^ 1` and `base ^ 2` both reverse-decode
        // to valid labels, so the cycle is not accidentally broken by a
